...
*  @endlicense
*  @brief Implementation file for Matrix class: real member functions
...
Matrix::Matrix(size_t _Rnum, size_t _Cnum, const std::vector<Real>& _elem, bool _diag, bool _ongpu, bool src_ongpu): Block(RTYPE, _Rnum, _Cnum, _diag, _ongpu){
  try{
    ...
    // set ongpu befor use init(Real* _elem, bool src_ongpu);
...
Matrix& Matrix::resize(rflag tp, size_t row, size_t col){
  try{
    ...
    if(diag){
      ...
      if(_elemNum > elemNum()){
        bool des_ongpu = ongpu;
        Real* elem;
        if(GMODE == 1)
          elem = (Real*)elemAlloc(_elemNum * sizeof(Real), des_ongpu); //May change the des_ongpu flag.
        else
          elem = (Real*)elemAllocForce(_elemNum * sizeof(Real), des_ongpu);
      ...
      if(col == Cnum){
        ...
        if(row > Rnum){
          ...
          Real* elem;
          if(GMODE == 1)
            elem = (Real*)elemAlloc(_elemNum * sizeof(Real), des_ongpu);
          else
            elem = (Real*)elemAllocForce(_elemNum * sizeof(Real), des_ongpu);
          elemBzero(elem, _elemNum * sizeof(Real), des_ongpu);
          elemCopy(elem, m_elem, elemNum() * sizeof(Real), des_ongpu, ongpu);
        ...
        Real* elem;
        if(GMODE == 1)
          elem = (Real*)elemAlloc(row * col * sizeof(Real), des_ongpu);
        else
          elem = (Real*)elemAllocForce(row * col * sizeof(Real), des_ongpu);
