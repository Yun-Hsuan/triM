...
public:
    ...
    Matrix(size_t _Rnum, size_t _Cnum, const Real* _elem, bool _diag=false, bool _ongpu=ONGPU, bool src_ongpu=false);
    /// @overload
    Matrix(size_t _Rnum, size_t _Cnum, const std::vector<Real>& _elem, bool _diag=false, bool _ongpu=ONGPU, bool src_ongpu=false);
    // @overload
    Matrix(rflag _tp, const std::string& fname);
    // @overload
    Matrix(rflag _tp, size_t _Rnum, size_t _Cnum, bool _diag=false, bool _ongpu=ONGPU);
    ...
    ///
    /// Returns the maximum element in Matrix
    Real max(rflag _tp, bool _ongpu=ONGPU);
    ...
    /// Allocate memory of size <tt> Rnum * Cnum </tt> ( or <tt> min(Rnum, Cnum)</tt> if \c diag is \c true) for
    /// matrix elements and copy the elements from \c _elem
    Matrix(size_t _Rnum, size_t _Cnum, const Complex* _elem, bool _diag=false, bool _ongpu=ONGPU, bool src_ongpu=false);
    /// @overload
    Matrix(size_t _Rnum, size_t _Cnum, const std::vector< Complex >& _elem, bool _diag=false, bool _ongpu=ONGPU, bool src_ongpu=false);

    Matrix(cflag _tp, const std::string& fname);

    Matrix(cflag _tp, size_t _Rnum, size_t _Cnum, bool _diag=false, bool _ongpu=ONGPU);
